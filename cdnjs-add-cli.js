...
const octokit = new (Octokit.plugin(require("octokit-create-pull-request")))({
...
const explore = async (path) => {
  const pathInput = await input(
    chalk.magenta.italic("Path to explore in package (blank to end): ")
  );
  if (pathInput) {
    // List files
    try {
...
const allFileMapFiles = (path, fileMap) => {
  ...
  for (const map of fileMap) {
    for (const file of map.files) {
      try {
        files.push(
          ...glob(file, { cwd: join(path, map.basePath), nodir: true })
        );
      } catch (_) {}
    }
  }
...
const createPR = async (data, body = "") => {
  ...
  const files = {};
  files[
    `packages/${data.name.slice(0, 1).toLowerCase()}/${data.name}.json`
  ...
  return await octokit.createPullRequest({
    ...
    title: name,
    body: `Adding ${data.name} using ${
      data.autoupdate.source
    } auto-update from ${data.autoupdate.target}.${
      body.length ? "\n\n" : ""
    }${body}`,
...
const updateChoice = async () => {
  while (true) {
    try {
      ...
      console.error(
        chalk.red("Please select either '1' or '2' for auto-update.")
      );
    } catch (_) {}
  }
};
...
const exploreAndGlob = async (path) => {
  ...
  while (true) {
    const basePath = await input(
      chalk.cyan.bold("Base path to use in file map (blank to end): ")
    );
    ...
    if (!basePath) {
      if (!fileMap.length) {
        console.error(
          chalk.red(
            "At least one file map is required for a library to auto-update"
          )
        );
      } else {
        break;
      ...
      while (true) {
        ...
        if (!globInput) {
          if (!patterns.length) {
            console.error(
              chalk.red(
                "At least one glob pattern is required for a base path in the file map"
              )
            );
          } else {
            break;
...
const chooseDefault = async (path, fileMap, version) => {
  ...
  return await input(
    chalk.cyan.bold(`Default file to highlight for usage (blank to skip): `)
  );
};
...
const npm = async (cdnjsData) => {
  ...
  const npmName =
    (
      await input(
        chalk.cyan.bold(`\nNPM package name (blank for ${cdnjsData.name}): `)
      )
    ).trim() || cdnjsData.name;
  ...
  const defaultFile = await chooseDefault(
    fullPath,
    cdnjsData.autoupdate.fileMap,
    jsonData["dist-tags"].latest
  );
  if (defaultFile) {
...
const githubRepo = async () => {
  while (true) {
    try {
      ...
      console.error(
        chalk.red("Could not parse a valid GitHub.com repo from given input.")
      );
    } catch (_) {}
  }
};
...
const github = async (cdnjsData) => {
  ...
  if (!repoTags.data || !repoTags.data.length) {
    console.error(
      chalk.red(
        `No tags found in repository https://github.com/${repoOwner}/${repoName}`
      )
    );
    return;
  }
  ...
  // Store basic info
  cdnjsData.description =
    repoData && repoData.data && repoData.data.description;
  cdnjsData.keywords = repoTopics && repoTopics.data && repoTopics.data.names;
  cdnjsData.author = repoOwner;
  cdnjsData.license =
    repoData &&
    repoData.data &&
    repoData.data.license &&
    repoData.data.license.spdx_id;
  cdnjsData.homepage =
    (repoData && repoData.data && repoData.data.homepage) ||
  ...
  // Ack
  console.log(
    `Downloaded ${repoOwner}/${repoName}@${repoTags.data[0].name}...\n`
  );
...
const main = async () => {
  ...
  // Get PR resolves
  const resolves = await input(
    chalk.cyan.bold(`Issue this PR resolves (blank to skip): `)
  );
  ...
  try {
    ...
    console.log(
      chalk.green.bold(
        `\n\nCreate new file on cdnjs/packages: packages/${cdnjsData.name
          .slice(0, 1)
          .toLowerCase()}/${cdnjsData.name}.json`
      )
    );
